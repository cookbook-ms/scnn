...
class MySCNN(nn.Module):
    ...
    def forward(self, Ls, Ds, adDs, xs):
        ...
        # tau = 0.01
        # # other ODD nonlinearity
        # out0_2 = self.C0_2(Ls[0], torch.sign(out0_1)*nn.ReLU()(out0_1-tau)) #+ self.D10_2(nn.LeakyReLU()(out1_1))
        # out1_2 = self.C1_2(Ls[1], torch.sign(out1_1)*nn.ReLU()(out1_1-tau)) #+ self.D01_2(nn.LeakyReLU()(out0_1)) + self.D21_2(nn.LeakyReLU()(out2_1))
        # out2_2 = self.C2_2(Ls[2], torch.sign(out2_1)*nn.ReLU()(out2_1-tau)) #+ self.D12_2(nn.LeakyReLU()(out1_1))
        # out3_2 = self.C3_2(Ls[3], torch.sign(out3_1)*nn.ReLU()(out3_1-tau)) #+ self.D12_2(nn.LeakyReLU()(out1_1))
        # out4_2 = self.C4_2(Ls[4], torch.sign(out4_1)*nn.ReLU()(out4_1-tau)) #+ self.D12_2(nn.LeakyReLU()(out1_1))
        # out5_2 = self.C5_2(Ls[5], torch.sign(out5_1)*nn.ReLU()(out5_1-tau)) #+ self.D12_2(nn.LeakyReLU()(out1_1))

        # out0_3 = self.C0_3(Ls[0], torch.sign(out0_2)*nn.ReLU()(out0_2-tau)) #+ self.D10_3(nn.LeakyReLU()(out1_2))
        # out1_3 = self.C1_3(Ls[1], torch.sign(out1_2)*nn.ReLU()(out1_2-tau)) #+ self.D01_3(nn.LeakyReLU()(out0_2)) + self.D21_2(nn.LeakyReLU()(out2_2))
        # out2_3 = self.C2_3(Ls[2], torch.sign(out2_2)*nn.ReLU()(out2_2-tau)) #+ self.D12_3(nn.LeakyReLU()(out1_2))
        # out3_3 = self.C3_3(Ls[3], torch.sign(out3_2)*nn.ReLU()(out3_2-tau)) #+ self.D12_3(nn.LeakyReLU()(out1_2))
        # out4_3 = self.C4_3(Ls[4], torch.sign(out4_2)*nn.ReLU()(out4_2-tau)) #+ self.D12_3(nn.LeakyReLU()(out1_2))
        # out5_3 = self.C5_3(Ls[5], torch.sign(out5_2)*nn.ReLU()(out5_2-tau)) #+ self.D12_3(nn.LeakyReLU()(out1_2))
...
def main():
    ...
    for m in range(num_realizations):
        ...
        for d in range(0, topdim+1):
            actuallogf_all = open("%s/actual_%d_all.txt" %(logdir, d), "w") # only the sampled underlying true data
            #print(torch.eq(cochain_target_alldegs[d][0, 0, masks[d]] , cochain_input_alldegs[d][0,0,:]))
